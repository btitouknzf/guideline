...
| 以下はその機能を用いた、推奨アクセスフローの概要である。（SOAPサーバについてはJavaEEサーバのJAX-WS実装を使用する。）
    ...
    * - | (4)
      - | プロキシが、SOAPサーバのWebServiceインターフェースを呼び出す。
        ...
        .. Note::
            ...
            ここではSOAPサーバの実装にJavaEEサーバのJAX-WS実装を使用するため、SpringMVCの機能（DispatcherServletを利用する機能など）については利用できない点に留意されたい。
...
.. warning::
    ...
    SOAPサーバのWebサービスはAPサーバのJAX-WS実装を利用しているため、利用するAPサーバによってWebサービスの実装、動作に違いが出る可能性があるので注意すること。
    
    例えば以下のような動作の違いが出る可能性がある。
    
    * HTTPステータスコードに応じて発生する例外が異なる
    
    意図しない動作となった場合には、本実装例を利用するAPサーバに応じて適宜カスタマイズされたい。
